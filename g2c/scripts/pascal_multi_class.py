...
#print(f'first trn_fns k: {list(trn_fns.keys())[0]}, v: {list(trn_fns.values())[0]}')
#print(f'trn_ids first: {trn_ids[0]}')
...
def get_y(bbox, clas):
    #print(f'get_y() len(bbox): {len(bbox)}, len(clas): {len(clas)}, bbox: {bbox}, clas: {clas}')
    #note this removes zero padding
    bbox = bbox.view(-1, 4)/sz
    bb_keep = ((bbox[:, 2]-bbox[:, 0]) > 0).nonzero()[:, 0]
    #print(f'get_y() bb_keep: {bb_keep}, clas: {clas[bb_keep]}')
...
def ssd_loss(pred,targ, print_it=False):
    ...
    for b_c, b_bb, bbox, clas in zip(*pred,*targ):
        #print(f'b_c: {b_c}, b_bb: {b_bb}, bbox: {bbox}, clas: {clas}')
...
plt.savefig(f'figs/pascal_multi_anchor_{SHORT_CYCLE_LEN}_{CYCLE_LEN}')
plt.close()


fig, ax = plt.subplots(figsize = (7,7))
torch_gt(ax, ima, anchor_cnr, b_clasi.max(1)[1])
plt.savefig(f'figs/pascal_multi_anchor_max_{SHORT_CYCLE_LEN}_{CYCLE_LEN}')
plt.close()
...
plt.savefig(f'figs/pascal_multi_jaccard_{SHORT_CYCLE_LEN}_{CYCLE_LEN}')
plt.close()
...
plt.tight_layout()
plt.savefig(f'figs/pascal_multi_final_{SHORT_CYCLE_LEN}_{CYCLE_LEN}')
plt.close()
...
for i, ax in enumerate(axes.flat):
    ...
    draw_text(ax, (0,0), b)
plt.tight_layout()
plt.savefig(f'figs/pascal_multi_classes_{SHORT_CYCLE_LEN}_{CYCLE_LEN}')
